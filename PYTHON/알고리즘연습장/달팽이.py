def solution(r, c):
    arr = [[0 for column in range(r)] for row in range(c)]
    # 상우하좌으로 움직일 수 있게 변화량 배열 생성 
    d_row = [-1, 0, 1, 0] # 행의 변화량 
    d_column = [0, 1, 0, -1] # 열의 변화량 

    # 변화량의 방향이 상우하좌으로 움직일 수 있게 하는 변수 선언
    ...
    while cnt <= total :
        ...
        if next_row < 0 or next_row >= c or next_column < 0 or next_column >= r or arr[next_row][next_column] > 0:
            # 방향 전환하고 이동 
